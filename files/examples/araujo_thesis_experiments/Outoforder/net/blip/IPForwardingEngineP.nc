...
#endif
 interface Timer<TMilli> as PrintTimerxx;
...
// ********************************* ATTACK ********************************* // 
    ...
    #define BUFFER_LENGTH 5 // lenght of the buffer
    
    struct ip6_hdr buffer_iph[BUFFER_LENGTH][BUFFER_LENGTH];
    uint16_t buffer_txflag = 0;
    uint8_t *buffer_payload[BUFFER_LENGTH][BUFFER_LENGTH] = { {NULL, NULL, NULL, NULL, NULL}, {NULL, NULL, NULL, NULL, NULL}, {NULL, NULL, NULL, NULL, NULL}, {NULL, NULL, NULL, NULL, NULL}, {NULL, NULL, NULL, NULL, NULL}};
    uint8_t buffer_time = 0;
    uint16_t timek = 0;
    uint8_t buffer_depth[BUFFER_LENGTH] = {0,0,0,0,0};
...
////    printf("\n");
  ...
  command error_t IPRaw.send(struct ip6_packet *pkt) {
    ...
    void asspacket(struct ip6_hdr *iph, void *payload,
                                             struct ip6_metadata *meta, uint8_t ifindex)  {
	...
	v.iov_len = len;
      ...
      if (!(signal ForwardingEvents.approve[next_hop_ifindex](&pkt, next_hop)))
        return;
////		printf("PRINTPKT: assemble packet done. Send! \n");
...
////	printf("BLIP: IPFWD: received packet\n");
...
// ********************************* ATTACK ********************************* //
	if (TOS_NODE_ID == 8) {
		...
		// Setup buffer index
		int packet_delay, buffer_index, payload_length, i, m, buffer_index_depth;
		buffer_time = (buffer_time + 1) % BUFFER_LENGTH;
		timek = timek + 1;
		if(timek <= BUFFER_LENGTH){		
			packet_delay = 0;}
		else{
		//packet_delay = ( call Random.rand16() ) % BUFFER_LENGTH;}
		packet_delay = 4;}
		buffer_index = (packet_delay + buffer_time) % BUFFER_LENGTH;
		buffer_index_depth = buffer_depth[buffer_index];

		//while (buffer_txflag & 1<<buffer_index) {
		//	printf("PRINTPKT: Buffer not empty\n");
		//	packet_delay = ( call Random.rand16() ) % BUFFER_LENGTH;
		//	buffer_index = (packet_delay + buffer_time) % BUFFER_LENGTH;
		//}
////		printf("PRINTPKT: ATTACK NODE: %d %d %d %d\n", packet_delay, buffer_time, buffer_index, buffer_index_depth);
		
		// Save data into buffer
		buffer_txflag |= 1<<buffer_index;
		buffer_iph[buffer_index][buffer_index_depth] = *iph;
		payload_length = ntohs(iph->ip6_plen);
////		printf("PRINTPKT: Payload: ");
////		for (i = 0; i<payload_length; ++i) {
////			printf("%d ", (int) *((uint8_t*) payload)+i);
////		}
////		printf("\n");
		
		buffer_payload[buffer_index][buffer_index_depth] = malloc(payload_length);
		memcpy(buffer_payload[buffer_index][buffer_index_depth], payload, payload_length);
		
		buffer_depth[buffer_index] = buffer_depth[buffer_index]+1; // increase the depth at the buffer_index position
////		printf("PRINTPKT: buffer_depth before: ");
		for (i = 0; i<buffer_depth[buffer_index]; ++i) {
			printf("%d ", (int) buffer_depth[i]);
		}
		printf("\n");
		// Read data from buffer
		if (buffer_txflag & 1<<buffer_time) { // Check if there is anything to send
////		printf("PRINTPKT: Send packets \n");
			for (m=0;m < buffer_depth[buffer_time]; ++m){ 
////				printf("PRINTPKT: sending packet %d\n", m);
				payload = buffer_payload[buffer_time][m];
				*iph = buffer_iph[buffer_time][m];
				buffer_txflag ^= 1<<buffer_time;
				asspacket(iph, payload, meta, ifindex);
				//call PrintTimerxx.startOneShot(50);
				//while (flagWAIT == 0){
				//}
				flagWAIT = 0;
				// burn time
			}
			for (m=0;m < buffer_depth[buffer_time]; ++m){ // free memory
				free(buffer_payload[buffer_time][m]);
			}
			buffer_depth[buffer_time] = 0;
////			printf("PRINTPKT: buffer_depth after: ");
			for (i = 0; i<buffer_depth[buffer_index]; ++i) {
				printf("%d ", (int) buffer_depth[i]);
			}
		printf("\n");
		} else { // Nothing to send
////			printf("PRINTPKT: Not sending anything\n");
			return;
		}
////		printf("PRINTPKT: Sending payload: ");
////		for (i = 0; i<payload_length; ++i) {
////			printf("%d ", (int) *((uint8_t*) payload)+i);
////		}
////		printf("\n");
...
// ********************************* ATTACK ********************************* //
    event void PrintTimerxx.fired() {
////	printf("PRINTPKT: TIMER FIRED!\n");
		flagWAIT = 1;
