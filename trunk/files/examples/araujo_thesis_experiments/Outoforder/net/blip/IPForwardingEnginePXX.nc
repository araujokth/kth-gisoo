...
// ********************************* ATTACK ********************************* // 
    ...
    struct ip6_hdr buffer_iph[BUFFER_LENGTH];
    uint16_t buffer_txflag = 0;
    uint8_t *buffer_payload[BUFFER_LENGTH] = {NULL, NULL, NULL, NULL, NULL};
...
void deep_copy(struct ip_iovec *from, struct ip_iovec *to) {
	// Copy iov_len
	to->iov_len = from->iov_len;
	// Allocate space for deep copy of iov_base
	to->iov_base = malloc(to->iov_len * sizeof(uint8_t));
	// Copy iov_base
	memcpy(to->iov_base, from->iov_base, to->iov_len * sizeof(uint8_t)); 
	// Check if we should copy iov_next
	if(from->iov_next != NULL) {
		// Allocate space for iov_next
		to->iov_base = malloc(sizeof(struct ip_iovec));
		// Recursive deep-copy of iov_next
		deep_copy(from->iov_next, to->iov_next);
	} else {
		to->iov_next = NULL;
	}
}
  ...
  event void IPForward.recv[uint8_t ifindex](struct ip6_hdr *iph, void *payload,
	...
	if (TOS_NODE_ID == 8) {
		...
		// Setup buffer index
		int packet_delay, buffer_index, payload_length, i;
		...
		printf("PRINTPKT: ATTACK NODE: %d %d %d\n", packet_delay, buffer_time, buffer_index);
		
		// Save data into buffer
		buffer_txflag |= 1<<buffer_index;
		buffer_iph[buffer_index] = *iph;
		...
		printf("\n");
		if (buffer_payload[buffer_index] != NULL) {
			free(buffer_payload[buffer_index]);
		}
		buffer_payload[buffer_index] = malloc(payload_length);
		memcpy(buffer_payload[buffer_index], payload, payload_length);
		
		// Read data from buffer
		if (buffer_txflag & 1<<buffer_time) { // Check if there is anything to send
			payload = buffer_payload[buffer_time];
			*iph = buffer_iph[buffer_time];
